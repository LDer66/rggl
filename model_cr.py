...
class GCN1(nn.Module):
    ...
    def con_loss(self, z1: torch.Tensor, z2: torch.Tensor, t):
        ...
        # print(f' pos1 = {refl_sim.sum()}   pos2 = {between_sim}')
        return -torch.log(
            between_sim.diag()
            / (refl_sim.sum(1) + between_sim.sum(1) - refl_sim.diag())).mean()
    
    def con_adj_loss(self, z1: torch.Tensor, z2: torch.Tensor, adj, t):
        f = lambda x: torch.exp(x / t)
        refl_sim = f(self.sim(z1, z1))
        between_sim = f(self.sim(z1, z2))
        # print(f'adj.shape = {adj.shape}  refl_sim.shape = {refl_sim.shape}  between_sim.shape = {between_sim.shape}')
        pos1 = torch.multiply(adj, refl_sim)
        pos2 = torch.multiply(adj, between_sim)
        return -torch.log(
            ( pos1.sum(1) + pos2.sum(1)) / (refl_sim.sum(1)  + between_sim.sum(1) - pos1.sum(1) - pos2.sum(1))
        ).mean()
    ...
    def forward_two_views(self, x, edge_index):
        ...
        x11 = self.projection(x1)
        x_1 = self.conv2(x1, edge_index_1)

        x2 = self.conv1(x2, edge_index_2)
        x2 = F.relu(x2)
        x2 = F.dropout(x2, p=self.dropout, training=self.training)
        x22 = self.projection(x2)
        x_2 = self.conv2(x2, edge_index_2)

        return x11, x22,x_1,x_2
